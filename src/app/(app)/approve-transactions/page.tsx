...
export default function ApproveTransactionsPage() {
  ...
  const getTransactionAmountDetails = (tx: PendingTransaction): string => {
    ...
    if ('count' in tx) {
        ...
        return `${shareTx.count} shares @ ${shareTx.valuePerShare.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Birr/share (Value: ${(shareTx.totalValueForAllocation || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Birr)`;
    }
    return 'N/A';
  };
