...
export default function ApproveTransactionsPage() {
  ...
  const getTransactionAmountDetails = (tx: PendingTransaction): string => {
    if ('amount' in tx) return `Birr ${(tx as Saving | Dividend).amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    if ('count' in tx) {
        const shareTx = tx as Share;
        return `${shareTx.count} shares @ Birr ${shareTx.valuePerShare.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}/share (Value: Birr ${(shareTx.totalValueForAllocation || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`;
    }
    return 'N/A';
  };
  ...
  const handleExport = () => {
    const dataToExport = pendingTransactions.map(tx => {
      let details = '';
      if ('amount' in tx) details = `Birr ${(tx as Saving | Dividend).amount.toFixed(2)}`;
      else if ('count' in tx) {
          const shareTx = tx as Share;
          details = `${shareTx.count} shares @ Birr ${shareTx.valuePerShare.toFixed(2)}/share`;
      }
      return {
