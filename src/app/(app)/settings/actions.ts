...
async function checkUserDuplicates(
  ...
  const OR = [];
  if (email)
    OR.push({ email: { equals: email, mode: 'insensitive' as const } });
  ...
  if (existingUser) {
    if (existingUser.email?.toLowerCase() === email.toLowerCase())
      return `Email is already in use by user ${existingUser.name}.`;
    if (existingUser.phoneNumber === phoneNumber)
      return `Phone number is already in use by user ${existingUser.name}.`;
  }

  const existingMember = await prisma.member.findFirst({ where: { OR } });
  if (existingMember) {
    if (existingMember.email?.toLowerCase() === email.toLowerCase())
      return `Email is already in use by member ${existingMember.fullName}.`;
    if (existingMember.phoneNumber === phoneNumber)
      return `Phone number is already in use by member ${existingMember.fullName}.`;
  }
...
export async function registerUserByAdmin(
  ...
  try {
    ...
    if (!/^0[79]\d{8}$/.test(data.phoneNumber)) {
      return {
        success: false,
        error: 'Phone number must be 10 digits and start with 07 or 09.',
      };
    }
    ...
    await logAudit('MEMBER_CREATE', {
      // Note: using MEMBER_CREATE for admin user creation as well for simplicity
      targetId: newUser.id,
      targetType: 'USER',
